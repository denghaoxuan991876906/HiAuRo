...
默认以"战斗事实轴 + 智能决策层 + 可视化编辑器"为核心。

## Core Value

以最平直的方式做出一个独立可用的 HiAuRo，让 ACR 作者能快速上手；事实轴等高级能力已内置，ACR 开发体验不受影响。
...
辅助轴**始终运行**，独立于模式切换。它包含安全坐标计算脚本（按副本编号加载），根据实时游戏状态计算角色应该去哪个位置。执行轴和事实轴切换时辅助轴不受影响，保证移动计算不断。
...
辅助轴（安全坐标计算）──→ 智能层·移动协调 ──→ 角色移动
```
- 战斗事实建模与个人执行彻底解耦
- 事实轴以 JSON 为权威产物
- `/hi fact` 命令切换至事实轴模式
- 决策引擎自动消费时间线事件并输出策略建议
- 移动协调释放辅助轴的移动需求，驱动角色自动走位
...
| **架构风格** | 平直、轻量、少包装、少跳转 |
| **注释语言** | 中文优先 |
| **接口稳定** | ACR 主接口尽量不变；新能力做 additive 扩展 |
| **事实轴产物** | 结构化 JSON（非手写 DSL） |
| **辅助轴** | 独立于模式切换始终运行，按副本加载坐标计算脚本 |
