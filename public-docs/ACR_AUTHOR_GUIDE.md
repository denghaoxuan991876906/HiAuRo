...
3. 在 **源码** 输入框中填写 `.cs` 文件的绝对路径。
4. 打开 **开发模式** 开关；开启时会立即编译并加载该文件。
...
插件重启时，如果配置仍启用了开发模式，只等待 Helper 初始化完成就会自动进行一次初始加载，不等待脱战或游戏数据就绪。切图期间可以完成编译并进入 Ready 状态；游戏数据恢复且当前职业可识别后，再加载匹配的 Rotation。这不是源码保存监听；运行期间修改 `.cs` 文件不会被自动监控，也不会自动重载。
