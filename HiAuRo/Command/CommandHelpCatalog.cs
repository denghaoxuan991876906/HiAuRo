...
public static class CommandHelpCatalog
{
    ...
    public static string MainHelpMessage =>
        "HiAuRo: /hi on|off|toggle|status|panel|reload|fact|assist [load|unload]|debug|gallery|catalog [export|upload]|target [on|off|toggle|status|logic|range|keep|dummy|countdown|hostile|death|aggro]|combat [status|range|knockback|dash|animlock]|showid [on|off|toggle|status]|buffinfo [on|off|toggle|status]|help|commands";
    ...
    [
        ...
        [
            ...
            new("/hi buffinfo on|off|toggle|status", "鼠标悬停 buff 时显示 StatusID", "/hi buffinfo toggle"),
#if DEBUG
