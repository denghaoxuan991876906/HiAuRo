...
public static class PositionalVfx
{
    ...
    public static void Update(float dt)
    {
        ...
        var canShowForTarget = target != null && ShouldShowForTarget(target);
        var showIndicator = cfg.ShowPositional && canShowForTarget && PositionalState.ActiveDir != PositionalDir.None;
    ...
    static bool ShouldShowForTarget(IGameObject target)
    {
        if (target is not IBattleChara bc) return false;
        if (!bc.IsTargetable || bc.IsDead == true) return false;
        if (!Data.Objects.Enemies.Exists(e => e.GameObjectID == target.GameObjectID)) return false;
        return Data.Me.DistanceToObject2D(target) <= 10f;
    }
