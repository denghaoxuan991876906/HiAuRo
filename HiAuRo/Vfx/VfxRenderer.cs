...
public sealed class VfxRenderer
{
    ...
    public IVfxZone ShowFan(Vector3 pos, float radius, FanAngle angle,
        float rotation = 0f, Vector4? color = null, float duration = -1f, string? tag = null)
    {
        var path = angle switch
        {
            FanAngle.Deg20  => VfxPath.Fan20,
            FanAngle.Deg30  => VfxPath.Fan30,
            FanAngle.Deg45  => VfxPath.Fan45,
            FanAngle.Deg60  => VfxPath.Fan60,
            FanAngle.Deg90  => VfxPath.Fan90,
            FanAngle.Deg100 => VfxPath.Fan100,
            FanAngle.Deg120 => VfxPath.Fan120,
            FanAngle.Deg150 => VfxPath.Fan150,
            FanAngle.Deg180 => VfxPath.Fan180,
            _               => VfxPath.Fan270,
        };
        return Show(path, pos, new Vector3(radius, 1f, radius), rotation, duration, tag, color);
    }

    public static FanAngle AngleFromDeg(float totalDeg)
    {
        var deg = (int)totalDeg;
        return deg switch
        {
            <= 20  => FanAngle.Deg20,
            <= 30  => FanAngle.Deg30,
    ...
    public IVfxZone ShowRingFan(Vector3 pos, float innerR, float outerR, FanAngle angle,
        float rotation = 0f, Vector4? color = null, float duration = -1f, string? tag = null)
    {
        var thickness = outerR - innerR;
