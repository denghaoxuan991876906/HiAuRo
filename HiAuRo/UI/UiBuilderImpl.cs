...
public sealed class UiBuilderImpl : IAcrUiBuilder
{
    ...
    private int _uidSeq;
    private string Uid(string label) => _currentTab + "_" + _currentGroup + "_" + _uidSeq++;
    ...
    {
        if (_isImGui) { if (InScope) CL.Divider(); }
    ...
    {
        if (_isImGui) { if (InScope) CL.Label(text); }
    ...
    {
        if (_isImGui) return InScope && CL.DefaultButton(label);
    ...
    public bool AddCheckbox(string label, ref bool value, string? expr = null)
    {
        if (_isImGui)
        {
            ...
            return RenderChanged(CL.Switch(Uid(label), label, ref value));
        }
    ...
    public bool AddSlider(string label, float min, float max, ref float value, string? expr = null)
    {
        ...
        {
            if (!InScope) return false;
            return RenderChanged(CL.Slider(Uid(label), label, ref value, min, max));
        }
    ...
    public bool AddDropdown(string label, string[] options, ref string value, string? expr = null)
    {
        if (_isImGui)
        {
            ...
            if (CL.Select(Uid(label), label, ref idx, options)) { value = options[idx]; Runtime.ACRLifecycle.MarkSettingsDirty(); return true; }
            return false;
        }
    ...
    public bool AddIntInput(string label, ref int value, int step = 1, int stepFast = 10, string? expr = null)
    {
        if (_isImGui)
        {
            if (!InScope) return false;
            var prev = value;
            if (CL.InputNumber(Uid(label), label, ref value, step, stepFast)) return RenderChanged(true);
            return false;
        }
    ...
    public bool AddFloatInput(string label, ref float value, string? expr = null)
    {
        ...
        {
            if (!InScope) return false;
            ImGui.TextColored(ImGuiLib.Theme.Colors.TextSecondary, label);
            ImGui.SameLine(160);
            using var c = new ImRaii.ColorDisposable();
            c.Push(ImGuiCol.FrameBg, ImGuiLib.Theme.Colors.FillSecondary);
            c.Push(ImGuiCol.FrameBgHovered, ImGuiLib.Theme.Colors.FillPrimary);
            c.Push(ImGuiCol.FrameBgActive, ImGuiLib.Theme.Colors.FillPrimary);
            c.Push(ImGuiCol.Text, ImGuiLib.Theme.Colors.TextPrimary);
            using var v = ImRaii.PushStyle(ImGuiStyleVar.FrameRounding, ImGuiLib.Theme.RadiusSM);
            ImGui.SetNextItemWidth(120);
            return RenderChanged(ImGui.InputFloat($"##{Uid(label)}", ref value));
        }
    ...
    public bool AddTextInput(string label, ref string value, string? expr = null)
    {
        ...
        {
            if (!InScope) return false;
            return RenderChanged(CL.InputText(Uid(label), label, ref value, 256, 200));
        }
