...
def create_nupkg(nuspec_path: str, build_out_dir: str, dalamud_home: str, output_dir: str, cli_version: str = "") -> str:
    ...
    ver = cli_version or meta.findtext("ns:version" if ns else "version", namespaces=ns) or meta.findtext("version", "0.1.0")
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print(f"用法: {sys.argv[0]} <nuspec路径> <HiAuRo输出目录> [DALAMUD_HOME] <输出目录> [--version VER]", file=sys.stderr)
        sys.exit(1)

    args, cli_version = parse_args(sys.argv[1:])
    nuspec = args[0]
    build_dir = args[1]
    if len(args) == 3:
        dalamud = ""
    ...
    try:
        result = create_nupkg(nuspec, build_dir, dalamud, out, cli_version)
